...
+(instancetype)gainInputAccessoryView:(id<HHZInputAccessoryViewDelegate>)delegate
{
    ...
    [btn addTarget:self action:@selector(completebtnClicked) forControlEvents:UIControlEventTouchUpInside];
    [btn setBackgroundColor:[UIColor colorWithRed:0/255.0 green:166/255.0 blue:228/255.0 alpha:1]];
    [vie addSubview:btn];

    UIButton * btn2 = [UIButton buttonWithType:UIButtonTypeCustom];
    btn2.frame = CGRectMake([UIScreen mainScreen].bounds.size.width - 90, (vie.bounds.size.height - 30)/2, 80, 30);
    ...
    vie.showTextLabel.frame = CGRectMake(90, (vie.bounds.size.height - 20)/2, [UIScreen mainScreen].bounds.size.width - 60, 20);
